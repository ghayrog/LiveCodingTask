...
while (consoleCommand?.ToUpper().Trim() != EXIT)
{
    ...
    switch (consoleCommand?.ToUpper().Trim().Split(" ")[0])
    {
        ...
        case CALC:
            ...
            //Для решения уравнения нужно использовать метод SolveFunctions из solver, где
            //f1 будет функция забойного давления (GetBottomPressure) скважины well, от аргументов Flow rate и Top node pressure;
            //f2 будет функция забойного давления (GetBottomPressure) пласта reservoir, от аргумента Flow rate.
